...
> 本文假设：**你几乎不会用 Linux**，但会用浏览器、会复制粘贴文字。  
> 全流程以 **腾讯云** 为例（阿里云、华为云思路一样，只是网页名字略不同）。  
> 你需要 **一台 Windows 电脑**（Mac 也可以，命令几乎一样）。
...
**时间规划**：备案没做完之前，你的域名 **不能正常当「正式网站」用**。比赛若很紧，请 **今天就开始买服务器和域名并提交备案**。等备案通过，再让评委访问 `https://你的域名`。
...
5. **时长**：轻量云常见 **最短就是 1 年**（购买页没有更短的也正常）。直接选 **1 年** 即可，比赛结束后机器还可以继续当练习环境用；若页面有 **新用户秒杀、学生优惠** 等更短套餐，以 **当下购买页显示为准**。  
...
2. 在 **域名注册** 里搜一个还没被注册的名字（例如和你队名、作品名相关）。  
3. 加入购物车 → 结账 → 按提示付 **年费**（通常几十元一年，以网页为准）。  
4. 买完后按提示做 **域名实名认证**（个人上传身份证即可）。**不实名，后面解析、备案会卡住。**
...
5. 登录成功后，提示符会变成类似 `root@VM-0-ubuntu:~#`，说明你已经进到服务器里了。
...
0. 若敲 `git` 提示找不到命令，先在服务器执行：**`sudo apt install -y git`**  
1. 在浏览器打开你的仓库页面，点 **克隆地址**，复制 **HTTPS** 链接。  
2. 在服务器执行（把地址换成你的）：  
...
## 第 20 步：让 Nginx 把访问转到你 docker 里的网站（关键）
...
1. **访问地址**：`https://你的域名/`  
2. **测试账号**：按你程序里的演示账号写清（学生在哪登、老师在哪登）。  
3. **若 AI 功能要 Key**：写明若失效可只看非 AI 功能（如有）。
...
- `deploy/docker-compose.yml`：一键启动数据库 + 后端 + 网站  
- `deploy/.env.example`：密码模板  

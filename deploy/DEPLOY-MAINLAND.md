...
3. **AI 功能**：评委环境需在服务器 `deploy/.env` 中配置 `OPENAI_API_KEY`（及按需的 `OPENAI_BASE_URL`、`OPENAI_MODEL`），并重建后端容器后才会启用；详见上文 **第 15 步 B**。若比赛方不要求演示 AI，可说明「未配置 Key 时 AI 入口会提示未启用，其它功能可正常查看」。

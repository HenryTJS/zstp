<script setup>
import KnowledgePointDiscussion from '../../../shared/components/KnowledgePointDiscussion.vue'
...
defineProps({
  ...
  enterReviewFromGraph: { type: Function, required: true },
...
<template>
  <article v-if="selectedNode && !isUnjoinedPreviewMode" class="result-card student-kp-detail-root">
    <header class="student-kp-hero">
      ...
      <div class="student-kp-hero-actions student-node-action-row">
        ...
        <button type="button" class="nav-btn" :disabled="!practiceTestAllowed" @click="enterReviewFromGraph">
          错题与记录
        </button>
      </div>
    </header>
  ...
  <article v-if="selectedNode && !isUnjoinedPreviewMode" class="result-card student-kp-discussion-card ui-mt-12">
    <h3 class="portal-section-title">评论区</h3>
